...
import { getQuestionSetByIdAndStatus, publishQuestionSetById } from '../../services/questionSet';
...
const publishQuestionSet = async (req: Request, res: Response) => {
  ...
  const questionSetDeatils = await getQuestionSetByIdAndStatus(questionSet_id, { status: Status.DRAFT });
